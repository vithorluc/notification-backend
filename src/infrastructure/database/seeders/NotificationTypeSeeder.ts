...
export class NotificationTypeSeeder {
  async run(dataSource: DataSource) {
    const notificationTypeRepository = dataSource.getRepository(NotificationType);
    ...
    for (const type of notificationTypes) {
      const existingNotificationType = await notificationTypeRepository.findOne({ where: { name: type.name } });
